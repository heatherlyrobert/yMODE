...
ymode__unit             (char *a_question, int n)
{
   ...
   if      (strcmp (a_question, "stack"        )  == 0) {
      ...
      if (g_allow [n] == NULL)  strcpy  (r, "еж");
      else {
         if (strlen (g_allow [n]) <= 10)  sprintf (r, "е%.10sж", g_allow [n]);
         else                             sprintf (r, "е%.10s>", g_allow [n]);
      }
      sprintf (s, "е%.15sж", g_modes [n].desc);
      if (g_mesg  [n] == NULL)  strcpy  (t, "еж");
      else                      sprintf (t, "е%.15sж", g_mesg [n]);
      ...
      snprintf (unit_answer, LEN_FULL, "MODE status (%2d) : %c  е%sж  е%sж", n, g_modes [n].abbr, g_modes [n].expect, g_actual [n]);
   }
